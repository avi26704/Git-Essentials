...
* **git remote add < upstream link >** - Used to add upstream link
 
* **git log main..origin/main** - Lists out the log of all the commits of main branch of the origin

## Example Flow for new branch creation

&nbsp;  git clone https://github.com/xyz/pandas

&nbsp;  cd pandas

&nbsp;  echo "Hello Pandas" > hello.txt
...
Example scenario: You need to fix the site immediately. You can't use reset because other developers have already pulled your broken code; if you "delete" the history, their computers will get confused.
...
**git tag** is used to capture a point in history as being important. Most people use it to mark Release Points (e.g., v1.0, v2.1.4). 

A tag is a pointer to a specific commit that doesn't move. Unlike a branch (which moves every time you add a new commit), a tag stays exactly where you put it.
...
**git stash** is your "I'm not done yet, but I need to switch tasks" button. 

It’s a way to temporarily shelf your uncommitted changes so you can work on something else. Git takes your modified files (both staged and unstaged) and saves them in a secret internal stack. Your working directory then becomes "clean" (matching the last commit). When you’re ready, you "pop" the changes back out.
...
## Example flow for fetching and rebasing the branch

git add . - Stage all the changes

git commit -m < msg > - Commit the changes

git fetch origin main - Fetches the changes from the main branch of the origin but does not reflect them in files

git rebase origin main - Takes all your commits and puts it ahead of the fetched branch

    After Rebasing, If there are Merge conflicts, then resolve the conflicts and perform the following steps:

    git add < file > - Add the file in which the merge conflicts were resolved

    git rebase --continue - Continue the rebasing process

git push origin main - pushes the commits to the main branch.

Now a Merge Request (MR) can be raised to merge your branch with the origin's branch.

## Example flow for stashing the changes

git stash - Stash the changes made to the files

git pull origin main - Pull the main branch from origin to work on some different feature

git add . - Make the changes and then stage the files for commit

git commit -m "Feature updation after stash" - Commit the changes

git push origin main - Push the changes ( Raise a MR for merging the changes )

git stash pop - Pop the stashed changes and continue working.
